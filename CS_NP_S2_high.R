...
np_score_lm <- function(df, m_mu, c_val){#sigma_hat_table, 
...
system.time({
  for(sim in 1:n_sim){
    ...
    d_cal$R <- np_score_lm(d_cal, model_mu, c_threshold)#sigma_hat_table, 
    d_test$R <- np_score_lm(d_test, model_mu, c_threshold)#sigma_hat_table, 
