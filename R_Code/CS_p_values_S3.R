...
v_score <- function(df, df_y, m_mu){
  if(model == "misspecified"){
    pred_mu <- as.numeric(predict(m_mu, newdata = df))
  } else {
    pred_mu <- unlist(predict(m_mu, df, aggregate = TRUE))
  }
...
for(sim in 1:n_sim){
  ...
  if(model=="misspecified"){
    model_mu <- lm(y~poly(x,2), data = d_train[,c("x","y")]) # linear
  }else{
    #model_mu <- flexmix(y~poly(x,2), data = d_train[,c("x","y")], k=2) # correct
...
if(model=="misspecified"){
  saveRDS(
    ...
    file = "p_value_misspecified_sim4.rds"
  )
}else{
  saveRDS(
    list(
